...
public class FireGolem : MonoBehaviour, ISwordHittable
{
    PlayerStats playST;
    ...
    void Update()
    {
        ...
        if (Vector3.Magnitude(gameObject.transform.position - playerPosition) > awarenessRange)
        {
            isIdle = true;
            IdleMovement();
        }
        else if (Vector3.Magnitude(gameObject.transform.position - playerPosition) <= awarenessRange && Vector3.Magnitude(gameObject.transform.position - playerPosition) > meleeRange && !Attacking && !PhantomBreastEnabled)
        {
            isIdle = false;
            SwitchState(EnemyState.walk);

        }
        else if (Vector3.Magnitude(gameObject.transform.position - playerPosition) <= meleeRange && !PhantomBreastEnabled)
        {
            isIdle = false;
